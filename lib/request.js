...
var BEGIN_REQUEST_DATA_NO_KEEP_CONN = Buffer.from('\0\x01\0\0\0\0\0\0'); // FCGI_ROLE_RESPONDER && !FCGI_KEEP_CONN
var BEGIN_REQUEST_DATA_KEEP_CONN = Buffer.from('\0\x01\x01\0\0\0\0\0'); // FCGI_ROLE_RESPONDER && FCGI_KEEP_CONN
...
module.exports = function(client){
	...
	return function(params, cb){
		allocReq(function(err, conn, reqId, handlers){
			...
			cb(null, {
				abort: function(){
					conn.send(MSG_TYPE.FCGI_ABORT_REQUEST, reqId, Buffer.alloc(0));
				},
				stdin: stdin,
				stdout: stdout,
				stderr: stderr,
